...
```mermaid
flowchart LR
    A[1. Build people capability] --> B[2. Build / source infrastructure]
    B --> C[3. Plan and schedule supply + demand]
    C --> D[4. Define logistics requirements]
    D --> E[5. Build logistics capability]
    E --> F[6. Execute, monitor, and correct]
...
```mermaid
flowchart TD
    D[Demand analysis / forecast] --> SOP[S&OP / aggregate alignment]
    SOP --> MP[Master planning]
    MP --> MPS[Master schedule + rough-cut capacity]
    MPS --> MRP[Material requirements]
    MRP --> CRP[Detailed capacity requirements]
    MPS --> DRP[Distribution requirements]
    DRP --> L[Warehouse + transport capacity]
```
